...
def calc(vp_vec, dvp, sp_vec, dsp, g, samp, vs_vec, beta, resolution):
    ...
    dvq = np.linalg.norm(vq_vec, ord=2, axis=2).reshape(samp, resolution, 1) # (samp, resolution, 1)
    ...
    dsq = np.linalg.norm(sq_vec, ord=2, axis=2).reshape(samp, resolution, 1) # (samp, resolution, 1)
    ...
    extra_cos = np.sum( -vp_norm * sp_norm, axis=0, keepdims=True) #(1, resolution)
    cos_sum += extra_cos
    cos_sum = cos_sum.reshape(480, 640)

    extra_p = (1- np.power(g, 2)) /  (np.power((1 - 2 * g * extra_cos + g**2),1.5) ) #(1, resolution)
    extra_ans = np.exp(- beta * (dvp + dsp)) * extra_p / (np.power(dsp, 2)) # (1, resolution)
